...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Apply Target Encoding. The training set will only have information in its dates.
        # The same is true for the test set

        # Mirar de fer la mitjana fins la data actual
        transformed_train = self.tr_enc.fit_transform(X[X['date'] >= test_date][self.tr_attr],
                                                      X[X['date'] >= test_date]['sales'], 
                                                      smoothing=1.0)

        transformed_test = self.tr_enc.fit_transform(X[X['date'] < test_date][self.tr_attr],
                                                     X[X['date'] < test_date]['sales'],
                                                     smoothing=1.0)
...
class AddAutoLag(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if self.add_classification:
            X['classification'] = np.where(X.groupby('id')['sales'].shift(1) < X['sales'], 'Incrementa', 'NoIncrementa')
...
def preprocessing_pipeline(dataset,
    ...
    pipeline = Pipeline([
        ...
        ('add_lag', AddAutoLag(add_classification=classification)),
